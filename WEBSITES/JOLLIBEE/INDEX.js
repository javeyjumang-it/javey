...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    //// Function to handle the internal navigation links (Account, Orders, etc.)
function openSetting(settingName) {
    closeAllOffCanvas(); // Close the menu first
    // In a real application, this would open a specific setting panel or route the user.
    // For now, it provides feedback.
    alert(`Navigating to ${settingName.charAt(0).toUpperCase() + settingName.slice(1)} Settings... (Functionality Simulated)`);
}

function openAuthModal(mode = 'login') {
    closeAllOffCanvas(); // Close sidebars
