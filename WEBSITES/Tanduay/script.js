...
const products = [
  {
    name: 'Tanduay Silver',
    price: 500,
    img: 'images/tanduay-silver.png',
    desc: 'Light, smooth, perfect for cocktails.',
    category: 'Light Rum'
  },
  {
    name: 'Tanduay Gold',
    price: 700,
    img: 'images/tanduay-gold.png',
    desc: 'Rich and flavorful for premium sipping.',
    category: 'Premium Rum'
  },
  {
    name: 'Tanduay Rhum',
    price: 650,
    img: 'images/tanduay-rhum.png',
    desc: 'Classic heritage taste loved for generations.',
    category: 'Classic Rum'
  },
  {
    name: 'Tanduay Expert',
    price: 900,
    img: 'images/tanduay-expert.png',
    desc: 'Premium aged rum for true connoisseurs.',
    category: 'Aged Rum'
  }
];

// Cart Management
let cart = JSON.parse(localStorage.getItem('tanduayCart')) || [];
let promoCode = '';
...
function addToCart(name, price, qty = 1) {
  ...
  localStorage.setItem('tanduayCart', JSON.stringify(cart));
  updateCartUI();
...
function getProductImg(name) {
  ...
  return product ? product.img : 'images/tanduay-gold.png';
}
...
function applyPromo() {
  ...
  if (promoInput.value.toUpperCase() === 'WELCOME20') {
    promoCode = 'WELCOME20';
...
function handleRegister(e) {
  e.preventDefault();
  showNotification('Account created! Welcome to Tanduay! 🎉', 'success');
  closeAuthModal();
}
...
function handleCheckout(e) {
  ...
  setTimeout(() => {
    ...
    cart = [];
    localStorage.removeItem('tanduayCart');
    promoCode = '';
    closeCheckout();
    updateCartUI();
  }, 1500);
...
console.log('Tanduay Website JS Fixed & Loaded! 🛒✨');
